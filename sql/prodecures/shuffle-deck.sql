...
BEGIN
    ...
    INSERT INTO cardsInDeck	SELECT * FROM (SELECT id_card, id_deck FROM cards, decks WHERE id_deck = deckid ORDER BY id_card DESC LIMIT 36) as m1;
            
    /* Раздаем по 6 карт каждому игроку */

    SET player1 = (SELECT min(id_player) FROM players WHERE id_deck = deckid);
    SET player2 = (SELECT max(id_player) FROM players WHERE id_deck = deckid);
